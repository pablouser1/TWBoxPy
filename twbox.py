...
BASE_URL = "https://art.gametdb.com/ds/coverS/{region}/{ndsId}.png"
...
def downloadArt(ndsId: str, regions: list, output: str, verbose: bool)-> tuple:
    success = False
    code = -1

    i = 0
    while i < len(regions) and not success:
        region = regions[i]
        printVerbose("Trying to download {0}, {1}".format(ndsId, region), verbose)
        url = BASE_URL.format(region=region, ndsId=ndsId)
